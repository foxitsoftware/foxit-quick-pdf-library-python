  ...
  def __init__(self, pathOrFile = None):
    if pathOrFile is None:
      if 8 * struct.calcsize("P") == 32:
        dplFile = "DebenuPDFLibraryDLL1611.dll"
      else:
        dplFile = "DebenuPDFLibrary64DLL1611.dll"
    else:
      if os.path.isfile(pathOrFile):
         dplFile = pathOrFile
      if os.path.isdir(pathOrFile):
        if 8 * struct.calcsize("P") == 32:
          dplFile = os.path.join(pathOrFile, "DebenuPDFLibraryDLL1611.dll")
        else:
          dplFile = os.path.join(pathOrFile, "DebenuPDFLibrary64DLL1611.dll")

...
# Copyright (c) 1999 - 2018 Foxit Software

# DebenuPDFLibraryCPDylib1611.py
# Python import class
# http://www.debenu.com/
...
class PDFLibrary:
  ...
  def __init__(self, pathOrFile = None):
    if pathOrFile is None:
      if 8 * struct.calcsize("P") == 32:
        dplFile = "libDebenuPDFLibraryCPDylib1611.dylib"
      else:
        dplFile = "libDebenuPDFLibrary64CPDylib1611.dylib"
    else:
      if os.path.isfile(pathOrFile):
         dplFile = pathOrFile
      if os.path.isdir(pathOrFile):
        if 8 * struct.calcsize("P") == 32:
          dplFile = os.path.join(pathOrFile, "libDebenuPDFLibraryCPDylib1611.dylib")
        else:
          dplFile = os.path.join(pathOrFile, "libDebenuPDFLibrary64CPDylib1611.dylib")
    self.dpl = C.CDLL(dplFile)
    ...
    self.dpl.DPLGetMeasurementUnits.argtypes = [C.c_int]
    self.dpl.DPLGetMeasurementUnits.restype = C.c_int
  ...
  def GetMeasurementUnits(self):
    result = self.dpl.DPLGetMeasurementUnits(self.instanceID)
    return result
